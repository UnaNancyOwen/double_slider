/*
 ...
 Version: 2.7.0 + double slider trackbar
 ...
 The additional implementation parts are distributed under following license.
...
template <typename T>
bool trackbar(cv::Mat& theWhere, int theX, int theY, int theWidth, T *theValue_min, T *theValue_max, T theMin, T theMax, int theSegments = 1, const char *theLabelFormat = "%.1Lf", unsigned int theOptions = 0, T theDiscreteStep = 1);
...
template <typename T> // T can be any float type (float, double, long double)
bool trackbar(int theWidth, T *theValue_min, T *theValue_max, T theMin, T theMax, int theSegments = 1, const char *theLabelFormat = "%.1Lf", unsigned int theOptions = 0, T theDiscreteStep = 1);
...
namespace internal
{
    ...
    bool trackbar(cvui_block_t &theBlock, int theX, int theY, int theWidth, long double *theValue_min, long double *theValue_max, const TrackbarParams& theParams);
	inline void trackbarForceValuesAsMultiplesOfSmallStep(const TrackbarParams & theParams, long double *theValue);
    inline void trackbarForceValuesAsMultiplesOfSmallStep( const TrackbarParams & theParams, long double *theValue_min, long double *theValue_max );
    ...
    template <typename T> // T can be any numeric type (int, double, unsigned int, etc)
    bool trackbar(cv::Mat& theWhere, int theX, int theY, int theWidth, T *theValue_min, T *theValue_max, const TrackbarParams& theParams);
    ...
    template <typename num_type>
    bool trackbar(int theWidth, num_type *theValue_min, num_type *theValue_max, const TrackbarParams& theParams) {
        cvui_block_t& aBlock = internal::topBlock();

        long double aValueAsDouble_min = static_cast<long double>(*theValue_min);
        long double aValueAsDouble_max = static_cast<long double>(*theValue_max);
        bool aResult = internal::trackbar(aBlock, aBlock.anchor.x, aBlock.anchor.y, theWidth, &aValueAsDouble_min, &aValueAsDouble_max, theParams);
        *theValue_min = static_cast<num_type>(aValueAsDouble_min);
        *theValue_max = static_cast<num_type>(aValueAsDouble_max);

        return aResult;
    }

    template <typename num_type>
    bool trackbar(cv::Mat& theWhere, int theX, int theY, int theWidth, num_type *theValue_min, num_type *theValue_max, const TrackbarParams& theParams) {
        gScreen.where = theWhere;

        long double aValueAsDouble_min = static_cast<long double>(*theValue_min);
        long double aValueAsDouble_max = static_cast<long double>(*theValue_max);
        bool aResult = internal::trackbar(gScreen, theX, theY, theWidth, &aValueAsDouble_min, &aValueAsDouble_max, theParams);
        *theValue_min = static_cast<num_type>(aValueAsDouble_min);
        *theValue_max = static_cast<num_type>(aValueAsDouble_max);
...
namespace render {
    ...
    void trackbarHandle(cvui_block_t& theBlock, int theState, cv::Rect& theShape, double theValue_min, double theValue_max, const internal::TrackbarParams &theParams, cv::Rect& theWorkingArea);
    ...
    void trackbar(cvui_block_t& theBlock, int theState, cv::Rect& theShape, double theValue_min, double theValue_max, const internal::TrackbarParams &theParams);
...
template <typename num_type>
bool trackbar(cv::Mat& theWhere, int theX, int theY, int theWidth, num_type *theValue_min, num_type *theValue_max, num_type theMin, num_type theMax, int theSegments, const char *theLabelFormat, unsigned int theOptions, num_type theDiscreteStep ) {
    internal::TrackbarParams aParams = internal::makeTrackbarParams( theMin, theMax, theDiscreteStep, theSegments, theLabelFormat, theOptions );
    return trackbar<num_type>( theWhere, theX, theY, theWidth, theValue_min, theValue_max, aParams );
}

template <typename num_type>
bool trackbar( int theWidth, num_type *theValue_min, num_type *theValue_max, num_type theMin, num_type theMax, int theSegments, const char *theLabelFormat, unsigned int theOptions, num_type theDiscreteStep ) {
    internal::TrackbarParams aParams = internal::makeTrackbarParams( theMin, theMax, theDiscreteStep, theSegments, theLabelFormat, theOptions );
    return trackbar<num_type>( theWidth, theValue_min, theValue_max, aParams );
}
...
namespace internal
{
    ...
    {
        if(bitsetHas(theParams.options, TRACKBAR_DISCRETE) && theParams.step != 0.) {
            long double k_min = (*theValue_min - theParams.min) / theParams.step;
            long double k_max = (*theValue_max - theParams.min) / theParams.step;
            k_min = (long double)cvRound((double)k_min);
            k_max = (long double)cvRound((double)k_max);
            *theValue_min = theParams.min + theParams.step * k_min;
            *theValue_max = theParams.min + theParams.step * k_max;
        }
    }
    ...
    bool trackbar(cvui_block_t& theBlock, int theX, int theY, int theWidth, long double *theValue_min, long double *theValue_max, const TrackbarParams & theParams) {
        // If one value exceeds the other, rewrite to the swap value.
        if(*theValue_max < *theValue_min) {
            int32_t min = std::min(*theValue_min, *theValue_max);
            int32_t max = std::max(*theValue_min, *theValue_max);
            *theValue_min = min;
            *theValue_max = max;
        }
        ...
        cvui_mouse_t& aMouse = internal::getContext().mouse;
        cv::Rect aContentArea(theX, theY, theWidth, 45);
        long double aValue_min = *theValue_min;
        long double aValue_max = *theValue_max;
        bool aMouseIsOver = aContentArea.contains(aMouse.position);

        render::trackbar(theBlock, aMouseIsOver ? OVER : OUT, aContentArea, *theValue_min, *theValue_max, theParams);

        if(aMouse.anyButton.pressed && aMouseIsOver) {
            if(aMouse.buttons[cvui::LEFT_BUTTON].pressed) {
                *theValue_min = internal::trackbarXPixelToValue(theParams, aContentArea, aMouse.position.x);
            }
            else{
                *theValue_max = internal::trackbarXPixelToValue(theParams, aContentArea, aMouse.position.x);
            }

            if(bitsetHas(theParams.options, TRACKBAR_DISCRETE)) {
                if(aMouse.buttons[cvui::LEFT_BUTTON].pressed) {
                    internal::trackbarForceValuesAsMultiplesOfSmallStep(theParams, theValue_min);
                }
                else{
                    internal::trackbarForceValuesAsMultiplesOfSmallStep(theParams, theValue_max);
                }
            }
        }
        ...
        return ((*theValue_min != aValue_min) || (*theValue_max != aValue_max));
    }
...
namespace render
{
    ...
    void trackbarHandle(cvui_block_t& theBlock, int theState, cv::Rect& theShape, double theValue_min, double theValue_max, const internal::TrackbarParams &theParams, cv::Rect& theWorkingArea) {
        ...
        // Draw the rectangle representing the handle
        int aPixelX_min = internal::trackbarValueToXPixel(theParams, theShape, theValue_min);
        int aPixelX_max = internal::trackbarValueToXPixel(theParams, theShape, theValue_max);
        int aIndicatorWidth = 3;
        int aIndicatorHeight = 4;
        cv::Point aPoint1_min(aPixelX_min - aIndicatorWidth, aBarTopLeft.y - aIndicatorHeight);
        cv::Point aPoint2_min(aPixelX_min + aIndicatorWidth, aBarTopLeft.y + aBarHeight + aIndicatorHeight);
        cv::Rect aRect_min(aPoint1_min, aPoint2_min);
        cv::Point aPoint1_max(aPixelX_max - aIndicatorWidth, aBarTopLeft.y - aIndicatorHeight);
        cv::Point aPoint2_max(aPixelX_max + aIndicatorWidth, aBarTopLeft.y + aBarHeight + aIndicatorHeight);
        cv::Rect aRect_max(aPoint1_max, aPoint2_max);

        int aFillColor = theState == OVER ? 0x525252 : 0x424242;

        rect(theBlock, aRect_min, 0x212121, 0x212121);
        aRect_min.x += 1; aRect_min.y += 1; aRect_min.width -= 2; aRect_min.height -= 2;
        rect(theBlock, aRect_min, 0x515151, aFillColor);

        rect(theBlock, aRect_max, 0x212121, 0x212121);
        aRect_max.x += 1; aRect_max.y += 1; aRect_max.width -= 2; aRect_max.height -= 2;
        rect(theBlock, aRect_max, 0x515151, aFillColor);

        bool aShowLabel = internal::bitsetHas(theParams.options, cvui::TRACKBAR_HIDE_VALUE_LABEL) == false;

        // Draw the handle label
        if(aShowLabel) {
            cv::Point aTextPos_min(aPixelX_min, aPoint2_min.y + 11);
            sprintf_s(internal::gBuffer, theParams.labelFormat.c_str(), static_cast<long double>(theValue_min));
            putTextCentered(theBlock, aTextPos_min, internal::gBuffer);

            cv::Point aTextPos_max(aPixelX_max, aPoint2_max.y + 11);
            sprintf_s(internal::gBuffer, theParams.labelFormat.c_str(), static_cast<long double>(theValue_max));
            putTextCentered(theBlock, aTextPos_max, internal::gBuffer);
        }
    }
    ...
    void trackbar(cvui_block_t& theBlock, int theState, cv::Rect& theShape, double theValue_min, double theValue_max, const internal::TrackbarParams &theParams) {
        ...
        trackbarPath(theBlock, theState, theShape, theValue_min, theParams, aWorkingArea);
        trackbarPath(theBlock, theState, theShape, theValue_max, theParams, aWorkingArea);

        bool aHideAllLabels = internal::bitsetHas(theParams.options, cvui::TRACKBAR_HIDE_LABELS);
        bool aShowSteps = internal::bitsetHas(theParams.options, cvui::TRACKBAR_HIDE_STEP_SCALE) == false;

        if(aShowSteps && !aHideAllLabels) {
            trackbarSteps(theBlock, theState, theShape, theValue_min, theParams, aWorkingArea);
            trackbarSteps(theBlock, theState, theShape, theValue_max, theParams, aWorkingArea);
        }

        if(!aHideAllLabels) {
            trackbarSegments(theBlock, theState, theShape, theValue_min, theParams, aWorkingArea);
            trackbarSegments(theBlock, theState, theShape, theValue_max, theParams, aWorkingArea);
        }

        trackbarHandle(theBlock, theState, theShape, theValue_min, theParams, aWorkingArea);
        trackbarHandle(theBlock, theState, theShape, theValue_max, theParams, aWorkingArea);
    }
